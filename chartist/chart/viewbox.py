...
class ChartViewBox(pg.ViewBox):
    ...
    def wheelEvent(self, ev, axis=None):
        ...
        if horizontal or shift:
            ...
            self.chart.panX(-delta / 1200 * (xmax - xmin))
        else:
            self.chart.zoomX(0.9985 ** delta, center=self.mapToView(ev.pos()).x())
    ...
    def mouseDragEvent(self, ev, axis=None):
        ...
        if self.pans_y and self.chart.tool == "zoom_in":
            ...
            if ev.isStart():
                self.chart.beginZoomBox((start.x(), start.y()))
            self.chart.updateZoomBox((current.x(), current.y()))
            if ev.isFinish():
                self.chart.finishZoomBox((current.x(), current.y()))
            return
