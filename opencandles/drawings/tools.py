...
from opencandles.core.formatting import fmt_price
from opencandles.theme import DOWN, DRAW

Qt = QtCore.Qt

def draw_pens(color=DRAW):
...
class Drawing:
    ...
    def add(self, plot, item):
        plot.addItem(item, ignoreBounds=True)
        self.items.append((plot, item))
        return item

    def clicked(self, item, ev):
        if ev.button() == Qt.MouseButton.RightButton:
            ev.accept()
            self.chart.remove_drawing(self)
...
class HLineDrawing(Drawing):
    ...
    def price_tags(self):
        return [(self.line.value(), fmt_price(self.price), DRAW)]
...
class VLineDrawing(Drawing):
    ...
    def time_tags(self):
        return [(self.line.value(), self.chart.time_text(self.line.value()), DRAW)]
...
class SegmentDrawing(Drawing):
    ...
    def __init__(self, chart, p1, p2, pen, hover):
        ...
        self.roi = self.add(chart.price_plot, pg.LineSegmentROI(
...
class TrendDrawing(SegmentDrawing):
    def __init__(self, chart, p1, p2):
        super().__init__(chart, p1, p2, *draw_pens())
...
class FibDrawing(SegmentDrawing):
    ...
    def __init__(self, chart, p1, p2):
        ...
        for lvl, color in FIB_LEVELS:
            self.lines.append(self.add(chart.price_plot, pg.PlotCurveItem(pen=pg.mkPen(color, width=1.2))))
            self.labels.append(self.add(chart.price_plot, pg.TextItem(color=color, anchor=(0, 1))))
        self.changed()
    ...
    def changed(self):
        ...
        for (lvl, _), line, label in zip(FIB_LEVELS, self.lines, self.labels):
            ...
            label.setText(f"{lvl:g} ({fmt_price(price)})")
            label.setPos(xa, y)
...
class RectDrawing(Drawing):
    def __init__(self, chart, p1, p2):
        ...
        fill.setAlpha(40)
        (x, y), (w, h) = self._geom()
        self.roi = self.add(chart.price_plot, FilledRectROI(
...
class NoteText(pg.TextItem):
    ...
    def mouseClickEvent(self, ev):
        if ev.button() == Qt.MouseButton.RightButton:
            ev.accept()
            self.drawing.chart.remove_drawing(self.drawing)
...
class TextDrawing(Drawing):
    def __init__(self, chart, point, text):
        ...
        self.item = self.add(chart.price_plot, NoteText(self, text))
        self.refresh()
...
class MeasureDrawing(Drawing):
    ...
    def __init__(self, chart, p1):
        super().__init__(chart)
        self.p1 = self.p2 = p1
        self.box = self.add(chart.price_plot, QtWidgets.QGraphicsRectItem())
        self.label = self.add(chart.price_plot, pg.TextItem(color="#ffffff", anchor=(0.5, 1)))
        self.update_end(p1)
