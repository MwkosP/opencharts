...
def draw_tag(painter, rect, text, background):
...
class TimeAxis(OffsetDateAxis):
    ...
    def paint(self, p, opt, widget):
        ...
        if vb is None:
            return
        p.setFont(tag_font())
        for xv, text, bg in self.chart.time_tags():
            x = self.mapFromScene(vb.mapViewToScene(QtCore.QPointF(xv, 0))).x()
            if 0 <= x <= self.width():
                draw_tag(p, QtCore.QRectF(x - 62, 3, 124, 18), text, bg)
    ...
    def mouseDragEvent(self, ev):
        ...
        self.chart.zoom_x(math.exp(-dx * 0.006))        # drag right = zoom in

    def wheelEvent(self, ev):
        ev.accept()
        self.chart.zoom_x(0.9985 ** ev.delta())
...
class ValueAxis(pg.AxisItem):
    ...
    def paint(self, p, opt, widget):
        ...
        if vb is None:
            return
        p.setFont(tag_font())
        for yv, text, bg in self.pane.value_tags():
            y = self.mapFromScene(vb.mapViewToScene(QtCore.QPointF(0, yv))).y()
            if 0 <= y <= self.height():
                draw_tag(p, QtCore.QRectF(1, y - 9, self.width() - 2, 18), text, bg)
...
class PriceAxis(ValueAxis):
    ...
    def paint(self, p, opt, widget):
        ...
        if vb is None:
            return
        p.setFont(tag_font())
        for yv, text, bg in self.chart.price_tags():
            y = self.mapFromScene(vb.mapViewToScene(QtCore.QPointF(0, yv))).y()
            if 0 <= y <= self.height():
                draw_tag(p, QtCore.QRectF(1, y - 9, self.width() - 2, 18), text, bg)
    ...
    def mouseDragEvent(self, ev):
        ...
        self.chart.scale_y(math.exp(dy * 0.006))         # drag down = squeeze

    def wheelEvent(self, ev):
        ev.accept()
        self.chart.scale_y(0.9985 ** ev.delta())

    def mouseClickEvent(self, ev):
        ev.accept()
        if ev.double():
            self.chart.set_auto(True)
